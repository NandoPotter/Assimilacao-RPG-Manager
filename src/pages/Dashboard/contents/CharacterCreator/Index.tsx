...
function CharacterCreatorBoard() {
    ...
    const finalizeCreation = async () => {
        ...
        try {
            ...
            // -----------------------------------------------------------
            // 1. POPULAR INVENTÁRIO (NOVO PASSO)
            // -----------------------------------------------------------
    ...
    // NOVO: Renderiza os Requisitos Coloridos
    ...
    const renderUpgradeRow = (label: string, key: string) => {
        const base = formData.aptidoesBase[key] || 0; const total = formData.aptidoes[key] || 0; const added = total - base;
        const nextCost = total === 0 ? '2XP' : total === 1 ? '4XP' : total === 2 ? '6XP' : 'MAX';
        return (
            <div className="stat-row" key={key}>
                <span className="stat-name">{label}</span>
                <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                    <div className="upgrade-math"><span className="math-base">{base}</span>+<span className={`math-added ${added > 0 ? 'active' : ''}`}>{added}</span>=<span className="math-total">{total}</span></div>
                    <div className="stat-control">
                        ...
                        <>
                            ...
                            <div className="form-section">
                                ...
                                <div className="stats-grid-container">
                                    <div><h4 className="subtitle-std">Pessoais</h4><input className="input-field mb-2" placeholder="Propósito 1" value={formData.p1} onChange={e => setFormData({...formData, p1: e.target.value})} /><input className="input-field" placeholder="Propósito 2" value={formData.p2} onChange={e => setFormData({...formData, p2: e.target.value})} /></div>
                                    <div><h4 className="subtitle-std">Coletivos</h4><input className="input-field mb-2" placeholder="Propósito 1" value={formData.c1} onChange={e => setFormData({...formData, c1: e.target.value})} /><input className="input-field" placeholder="Propósito 2" value={formData.c2} onChange={e => setFormData({...formData, c2: e.target.value})} /></div>
                                </div>
                            </div>
                        </>
                    )}
                    ...
                    {step === 2 && (
                        <>
                            <div className="form-section compact-section">
                                <div className="header-compact" style={{display:'flex', justifyContent:'space-between'}}><h3>Instintos</h3><span className="points-remaining">Pontos: {pontosInstintos}</span></div>
                            ...
                            <div className="form-section compact-section">
                                <div className="header-compact" style={{display:'flex', justifyContent:'space-between'}}><h3>Aptidões</h3><span className="points-remaining">Pontos: {pontosAptidoes}</span></div>
                                <div className="aptitudes-split-row compact-gap">
                                    <div className="compact-col"><h4 className="subtitle-std text-center">Conhecimentos</h4><div className="stats-list-vertical">{['biologia','erudicao','engenharia','geografia','medicina','seguranca'].map(k => renderStatRow(NAME_MAP[k], k, 'aptidoes'))}</div></div>
                                    <div className="compact-col"><h4 className="subtitle-std text-center">Práticas</h4><div className="stats-list-vertical">{['armas','atletismo','expressao','furtividade','manufaturas','sobrevivencia'].map(k => renderStatRow(NAME_MAP[k], k, 'aptidoes'))}</div></div>
                                </div>
                            </div>
                        </>
                    )}
                        ...
                        <div className="step3-grid">
                            <div className="step3-col-left">
                                <div className="form-section characteristics-container">
                                    <div className="header-compact"><h3>Características</h3></div>
                                            ...
                                            return (
                                                <div key={char.id} className={`carac-row ${isLocked ? 'locked' : ''}`} onClick={() => !isLocked && toggleCharacteristic(char.id, char.cost, char.requirements)}>
                                                    ...
                                                    <div className="carac-info">
                                                        ...
                                                        {/* ADICIONADO: Exibição dos requisitos */}
                            ...
                            <div className="step3-col-right">
                                ...
                                <div className="form-section expanded-section">
                                    <div className="header-compact"><h3>Evoluir Aptidões</h3><p className="evolution-hint">0→1 (2XP) | 1→2 (4XP) | 2→3 (6XP)</p></div>
                                    <div className="aptitudes-split-row compact-gap" style={{overflowY:'auto'}}>
                                        <div className="compact-col"><div className="stats-list-vertical">{['biologia','erudicao','engenharia','geografia','medicina','seguranca'].map(k => renderUpgradeRow(NAME_MAP[k], k))}</div></div>
                                        <div className="compact-col"><div className="stats-list-vertical">{['armas','atletismo','expressao','furtividade','manufaturas','sobrevivencia'].map(k => renderUpgradeRow(NAME_MAP[k], k))}</div></div>
                                    </div>
                                </div>
